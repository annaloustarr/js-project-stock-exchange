...
// Will get company profile if search is not empty and add to list to show user.
    ...
    fetch(
      ...
      .then(data => {
        ...
        for (let i = 0; i < companyList.length; i++) {
          ...
          companyName.textContent = companyList[i].name;
          companyName.classList.add("company-name");

          let companySymbol = document.createElement("a");
          companySymbol.setAttribute(
            "href",
            `/company.html?symbol=${mySymbol}`
          );
          companySymbol.textContent = " (" + companyList[i].symbol + ")";
